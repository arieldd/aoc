...
int part1(const rucksack_list_t &rucksacks) {
  ...
  for (auto r : rucksacks) {
    ...
    auto pocket1 = r.substr(0, half_size), pocket2 = r.substr(half_size);
...
int part2(std::vector<std::string> rucksacks) {
  ...
  for (int ix = 0; ix < length; ix += 3) {
    auto r1 = rucksacks[ix], r2 = rucksacks[ix + 1], r3 = rucksacks[ix + 2];
    ...
    std::set_intersection(set1.begin(), set1.end(), set2.begin(), set2.end(),
                          std::back_inserter(mid_step));

    std::set_intersection(mid_step.begin(), mid_step.end(), set3.begin(),
                          set3.end(), std::back_inserter(badges));
  }

  return std::accumulate(
      badges.begin(), badges.end(), 0,
      [](auto &sum, auto &r) { return sum + item_priority(r); });
}
