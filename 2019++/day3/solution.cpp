...
struct coord_t{
    ...
    coord_t operator+(const coord_t& other){
        coord_t ret {.i = i + other.i, .j = j + other.j, .steps = steps + other.steps};
...
std::vector<coord_t> get_intersection(const std::unordered_set<coord_t >& wire1, const std::unordered_set<coord_t >& wire2){
...
int main(){
    ...
    auto wire1 = follow_path(moves[0]),
         wire2 = follow_path(moves[1]);
